                ...
                {[1, 2, 3, 4].map(n => {
                  ...
                  return (
                    <button
                      ...
                      title={`Bad Time — ${label} → ขึ้นสถานะ TM ที่หลุมนี้ทันที`}
                      style={{
                        background: isFlagged ? "#ff6ec7" : "#1a0014",
                        border: `1px solid ${isFlagged ? "#ff6ec7" : "#ff6ec788"}`,
                        color: isFlagged ? "#1a0014" : "#ff6ec7",
                        borderRadius: 6, padding: "4px 10px", cursor: "pointer",
                        fontFamily: "inherit", fontSize: 11, fontWeight: 700,
...
function SummaryScreen({ groups, groupData, pars, parTimes, suspensions, isSuspended, pendingStopTime, totalOffsetMin, onBack, currentUser, onLogout }) {
  ...
  // "Total players timed" should count how many distinct players were actually flagged per
  // group (whether via a normal TM selection or a Bad Time press), not sum up every row —
  // since MN/TM auto-logs the same ongoing target again on every hole it stays active, so
  // summing raw rows would count the same player over and over. "All" is treated as covering
  // all 4 player slots (P1–P4) offered in the TM/Bad-Time controls.
  const totalPlayersTimed = groups.reduce((sum, g) => {
    const logs = (groupData[g.id]?.actionLogs || []).filter(l => l.type === "TM");
    const playerSet = new Set();
    logs.forEach(l => {
      if (l.target === "All") {
        [1, 2, 3, 4].forEach(n => playerSet.add(`P${n}`));
      } else {
        (l.target || "").split(",").map(s => s.trim()).filter(Boolean).forEach(p => playerSet.add(p));
      }
    });
    return sum + playerSet.size;
  }, 0);
