...
function GroupMonitor({ group, pars, parTimes, schedule, onUpdate, onBack, currentUser,
  ...
  // can run at the same time here). Works as an on/off toggle: pressing a player who is
  // already Bad-Timed undoes it (in case of a mis-click) instead of logging it again.
  const triggerBadTimeFor = (playerNum) => {
    const playerLabel = `P${playerNum}`;
    const isBadTimed = actionLogs.some(l => l.type === "TM" && l.badTime && l.target === playerLabel);

    if (isBadTimed) {
      // Toggle OFF — undo the Bad Time flag for this player: remove their Bad Time log(s)
      // and drop them back out of the current TM target (unless "All" is active, which
      // already covers everyone regardless of any one player's Bad Time flag).
      const nextLogs = actionLogs.filter(l => !(l.type === "TM" && l.badTime && l.target === playerLabel));
      let newTarget = tmTarget;
      let newTmActive = tmActive;
      let newTmName = tmName;
      if (tmTarget && tmTarget !== "All") {
        newTarget = tmTarget.split(",").map(s => s.trim()).filter(p => p && p !== playerLabel).join(", ");
        if (!newTarget) { newTmActive = false; newTmName = ""; }
      }
      setActionLogs(nextLogs);
      setTmTarget(newTarget);
      setTmActive(newTmActive);
      setTmName(newTmName);
      onUpdate({
        holeData, records, currentHole: currentSlot, actionLogs: nextLogs,
        mnActive, mnName,
        tmActive: newTmActive, tmName: newTmName, tmTarget: newTarget,
      });
      return;
    }
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                              ...
                              {logs.map((l, li) => (
                                <div key={li} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: logColor(l.type), background: `${logBg(l.type)}44`, borderRadius: 4, padding: "2px 4px", textAlign: "left", lineHeight: 1.4 }}>
                                  <span style={{ flex: 1 }}>
                                    {l.badTime ? (
                                      <><span style={{ fontWeight: 700 }}>⚡ TM Bad Time</span> {l.target}{l.name ? ` - ${l.name}` : ""}</>
                                    ) : (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            {tmRows.length > 0 && (
              <tfoot>
                ...
                <tr>
                  <td colSpan={5} style={{ ...tdS, textAlign: "right", color: "#666", fontSize: 11, borderBottom: "none" }}>⤷ via normal TM selection</td>
                  <td style={{ ...tdS, color: "#ff6ec7", fontSize: 11, borderBottom: "none" }}>{totalNormalTMPlayers}</td>
                </tr>
                <tr>
                  <td colSpan={5} style={{ ...tdS, textAlign: "right", color: "#666", fontSize: 11, borderBottom: "none" }}>⤷ via Bad Time</td>
                  <td style={{ ...tdS, color: "#ffd966", fontSize: 11, borderBottom: "none" }}>{totalBadTimePlayers}</td>
                </tr>
                <tr>
