...
function GroupMonitor({ group, pars, parTimes, schedule, onUpdate, onBack, currentUser,
  ...
  // "Bad Time" — quick action while MN is active: flags a specific player (P1-P4) and
                ...
                {[1, 2, 3, 4].map(n => {
                  ...
                  return (
                    <button
                      ...
                      title={`Bad Time — ${label} → ขึ้นสถานะ TM ที่หลุมนี้ทันที`}
                      style={{
                                  ...
                                  >
                                    ...
                                    {holeLogs.map((l, li) => (
                                      <div key={li} style={{ marginTop: 3, fontSize: 9, fontWeight: 700, color: logColor(l.type), background: `${logBg(l.type)}55`, borderRadius: 4, padding: "1px 4px" }}>
                                        {l.badTime ? `⚡ TM Bad Time ${l.target || ""}${l.name ? ` · ${l.name}` : ""}` : <>{l.type} {l.target ? `${l.target} · ` : ""}{l.name}</>}
                                      </div>
                                    ))}
                                    {showMnPreview && (
