                          ...
                          <tr key={g.id}>
                            ...
                            {order.map((hi, slot) => {
                              ...
                              if (!endTime || !startTime) {
                                const groupSlot = data?.currentHole ?? 0;
                                const showMnPreview = mnActiveNow && slot >= groupSlot && !holeLogs.some(l => l.type === "MN");
                                const showTmPreview = tmActiveNow && slot >= groupSlot && !holeLogs.some(l => l.type === "TM");
                                return (
                                  ...
                                  >
                                    ...
                                    {showTmPreview && (
                                      <div style={{ marginTop: 3, fontSize: 9, fontWeight: 700, color: "#ff6ec7", background: "#2a002055", border: "1px dashed #ff6ec755", borderRadius: 4, padding: "1px 4px" }}>
                                        ⏱ TM {data?.tmTarget ? `${data.tmTarget} · ` : ""}{data?.tmName}
                                      </div>
                                    )}
                                  </td>
                                );
                              ...
                              const groupSlotDone = data?.currentHole ?? 0;
                              const showMnPreviewDone = mnActiveNow && slot >= groupSlotDone && !holeLogs.some(l => l.type === "MN");
                              const showTmPreviewDone = tmActiveNow && slot >= groupSlotDone && !holeLogs.some(l => l.type === "TM");
                              return (
