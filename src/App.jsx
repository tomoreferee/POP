...
function GroupMonitor({ group, pars, parTimes, schedule, onUpdate, onBack, currentUser,
  ...
  // immediately turns on TM for that player starting from the NEXT hole, without
  // switching off the group's MN status (MN and TM can run at the same time here).
  const triggerBadTimeFor = (playerNum) => {
    if (currentSlot >= 17) return; // already on/after the last hole — no "next hole" to flag
    const nextSlot = currentSlot + 1;
    const nextHoleIdx = holeOrder[nextSlot];
    const deadline = (adjustedSchedule[nextHoleIdx] ?? 0) + (parTimes?.[nextHoleIdx] ?? 14);
    const diffAtLog = nowInMin() - deadline + 1;
                ...
                {[1, 2, 3, 4].map(n => {
                  ...
                  return (
                    <button
                      ...
                      title={`Bad Time — ${label} → เปิด TM ในหลุมถัดไป`}
                      style={{
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                              ...
                              {logs.map((l, li) => (
                                <div key={li} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, color: logColor(l.type), background: `${logBg(l.type)}44`, borderRadius: 4, padding: "2px 4px", textAlign: "left", lineHeight: 1.4 }}>
                                  <span style={{ flex: 1 }}>
                                    <span style={{ fontWeight: 700 }}>{l.badTime ? "⚡" : ""}{l.type}</span> {l.name}{l.target ? ` · ${l.target}` : ""}
                                  </span>
                                  <button
                    ...
                    <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 3, alignItems: "center" }}>
                      ...
                      {logs.map((l, li) => (
                        <span key={li} style={{
                          ...
                          {l.badTime ? "⚡" : ""}{l.type} H{l.holeIdx + 1}
                          <button
