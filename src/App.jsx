...
const SUPABASE_ANON_KEY = "YeyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFxem1wa3BvYW5sdHhxemZlb3pmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI5ODg0MTYsImV4cCI6MjA5ODU2NDQxNn0.20p6PlzoAjy8GSTF0pbzeZt9r6t6AgrPLzjIH5sSeK8";
...
function GroupMonitor({ group, pars, parTimes, schedule, onUpdate, onBack, currentUser,
  ...
  // "Bad Time" — quick action while MN is active: flags a specific player (P1-P4) at the
  // CURRENT hole (the Bad Time log shows immediately on the hole it was pressed on), and
  // turns on TM for that player starting from the NEXT hole, without switching off the
  // group's MN status (MN and TM can run at the same time here).
                ...
                {[1, 2, 3, 4].map(n => {
                  ...
                  return (
                    <button
                      ...
                      title={`Bad Time — ${label} → ขึ้น Bad Time หลุมนี้ และ TM หลุมถัดไป`}
                      style={{
                        background: isFlagged ? "#ff6ec7" : "#2a0020",
                        border: `1px solid ${isFlagged ? "#ff6ec7" : "#ff6ec788"}`,
                        color: isFlagged ? "#1a0014" : "#ff6ec7",
...
function SummaryScreen({ groups, groupData, pars, parTimes, suspensions, isSuspended, pendingStopTime, totalOffsetMin, onBack, currentUser, onLogout }) {
  ...
  const totalPlayersTimed = tmRows.reduce((sum, r) => sum + r.players.length, 0);
        ...
        <div style={{ background: "#141626", border: "1px solid #2a2d4a", borderRadius: 12, padding: "4px 0", overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {tmRows.map((r, i) => (
                <tr key={i}>
                  <td style={{ ...tdS, textAlign: "left", fontWeight: 700 }}>{r.groupName}</td>
                  <td style={tdS}>H{r.holeIdx + 1}</td>
                  <td style={{ ...tdS, color: "#ff6ec7", fontWeight: 700 }}>{r.players.join(", ")}</td>
                  <td style={tdS}>{r.time}</td>
                  <td style={tdS}>{r.by}</td>
                  <td style={tdS}>{r.badTime ? "⚡ Yes" : "–"}</td>
                </tr>
              ))}
              {tmRows.length === 0 && (
                <tr><td colSpan={6} style={{ ...tdS, color: "#555", padding: 20 }}>No TM records yet</td></tr>
              )}
            </tbody>
            {tmRows.length > 0 && (
              <tfoot>
                <tr>
                  <td colSpan={5} style={{ ...tdS, textAlign: "right", color: "#8899cc", fontWeight: 700, borderBottom: "none" }}>Total players timed (TM)</td>
                  <td style={{ ...tdS, color: "#ff6ec7", fontWeight: 700, borderBottom: "none" }}>{totalPlayersTimed}</td>
                </tr>
                <tr>
                  <td colSpan={5} style={{ ...tdS, textAlign: "right", color: "#8899cc", fontWeight: 700, borderBottom: "none" }}>Total Bad Time triggers</td>
                  <td style={{ ...tdS, color: "#ffd966", fontWeight: 700, borderBottom: "none" }}>{totalBadTime}</td>
                </tr>
              </tfoot>
            )}
          </table>
