...
function GroupMonitor({ group, pars, parTimes, schedule, onUpdate, onBack, currentUser,
  ...
  const sortedGroups = useMemo(() => {
    const toMin = t => { const [h, m] = (t || "0:00").split(":").map(Number); return h * 60 + m; };
    return (allGroups || []).slice().sort((a, b) => toMin(a.startTime) - toMin(b.startTime));
  }, [allGroups]);
  ...
  return (
    <div style={{ background: bgColor, minHeight: "100vh", fontFamily: "'IBM Plex Mono', monospace", color: "#eee", transition: "background 1s" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 20px", background: "#141626cc", borderBottom: "1px solid #2a2d4a", backdropFilter: "blur(8px)" }}>
        ...
        {onSwitchGroup && (
          <button
            onClick={() => prevGroup && onSwitchGroup(prevGroup)}
            disabled={!prevGroup}
            title="กลุ่มก่อนหน้า"
        ...
        {onSwitchGroup && (
          <button
            onClick={() => nextGroup && onSwitchGroup(nextGroup)}
            disabled={!nextGroup}
            title="กลุ่มถัดไป"
