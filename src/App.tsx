...
const SUPABASE_URL = "https://qqzmpkpoanltxqzfeozf.supabase.co/rest/v1/";
    ...
    order.forEach(hi => {
      ...
      if (hd.startTime && hd.endTime) {
        if (hd.manualDiff !== undefined) {
          ...
          const [eh, em] = hd.endTime.split(":").map(Number);
          const actual = (eh * 60 + em) - (sh * 60 + sm);
          diff = Math.round(actual - (parTimes?.[hi] ?? 14)) + 1;
        }
      }
      detailRows.push({
        Group: g.name,
        ...
        Status: diff !== null ? { ok: "On time", warn: "Less OOP", late: "OOP" }[getStatus(diff)] : "",
      });
    });
...
function QuickGeneratePanel({ onGenerate, existingGroups1, existingGroups10 }) {
  ...
  const handleGenerate = () => {
    ...
    const existingAll = [...existingGroups1, ...existingGroups10];
    ...
    <div style={{ background:"#141626", border:"1px solid #4e9af133", borderRadius:12, padding:"18px 20px", marginBottom:16 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
        ...
        <div style={{ display:"flex", gap:6 }}>
          {[["h1only","🟢 H1 only"],["h10only","🔵 H10 only"],["both","🟢 H1 + 🔵 H10"]].map(([m,label]) => (
...
function SetupScreen({ onStart, currentUser, isAdmin, onManageUsers, onLogout, onClearSession }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0f1a", color: "#eee", fontFamily: "'IBM Plex Mono', monospace" }}>
      ...
      <div style={{ background: "#141626", borderBottom: "1px solid #2a2d4a", padding: "14px 24px", display: "flex", alignItems: "center", gap: 16 }}>
        <div>
          <div style={{ fontSize: 28, fontFamily: "'Bebas Neue'", letterSpacing: 4, color: "#4e9af1" }}>⛳ PACE MONITOR</div>
        ...
        <div style={{ marginLeft: "auto", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          ...
          {/* Row 2: action buttons + save badge */}
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {isAdmin && (
              <button
            ...
            <span style={{ fontSize: 10, color: "#6effa0", background: "#0a1a0a", border: "1px solid #6effa033", borderRadius: 5, padding: "3px 8px" }}>💾 Auto-saved</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "24px 24px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 24 }}>
          ...
          {/* ─── Shotgun Start Panel (4 holes, use when necessary) ───────────────────── */}
          {isAdmin && (
            <ShotgunStartPanel
              pars={pars}
              onGenerate={handleGenerateShotgun}
              existingGroups={[...groups1, ...groups10, ...groupsShotgun]}
            />
          )}
        ...
        <button
          onClick={() => { if (isAdmin || allGroups.length > 0) onStart(allGroups, pars, parTimes); }}
...
function GroupMonitor({ group, pars, parTimes, schedule, onUpdate, onBack, currentUser,
  isSuspended, suspensions, totalOffsetMin, pendingStopTime, onLogout }) {
  const initHoleData = () =>
  ...
  const confirmEditDiff = (i) => {
    ...
    commitRecord(i, { ...(hd ?? {}), endTime: newEndTime });
    setEditingDiff(null);
  };
  ...
  const targetLabel = (targets) => {
    if (!targets || targets.length === 0 || targets.includes("ALL")) return "All";
  ...
  const markHole = () => {
    ...
    let nxtLogs = actionLogs;
    if (mnActive) {
      const alreadyMN = nxtLogs.some(l => l.type === "MN" && l.holeIdx === currentHole);
      if (!alreadyMN) {
        const autoDeadline = (adjustedSchedule[currentHole] ?? 0) + (parTimes?.[currentHole] ?? 14);
        const autoDiff = nowInMin() - autoDeadline + 1;
        nxtLogs = [...nxtLogs, { holeIdx: currentHole, type: "MN", name: mnName, time: minToTime(nowInMin()), diff: autoDiff, auto: true }];
      }
    }
    if (tmActive) {
      const alreadyTM = nxtLogs.some(l => l.type === "TM" && l.holeIdx === currentHole);
      if (!alreadyTM) {
        const autoDeadline = (adjustedSchedule[currentHole] ?? 0) + (parTimes?.[currentHole] ?? 14);
        const autoDiff = nowInMin() - autoDeadline + 1;
        nxtLogs = [...nxtLogs, { holeIdx: currentHole, type: "TM", name: tmName, time: minToTime(nowInMin()), diff: autoDiff, target: tmTarget, auto: true }];
      }
    }
  ...
  return (
    <div style={{ background: bgColor, minHeight: "100vh", fontFamily: "'IBM Plex Mono', monospace", color: "#eee", transition: "background 1s" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 20px", background: "#141626cc", borderBottom: "1px solid #2a2d4a", backdropFilter: "blur(8px)" }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "#4e9af1", cursor: "pointer", fontSize: 18, flexShrink: 0 }}>←</button>
        <div style={{ width: 10, height: 10, borderRadius: "50%", background: group.color, flexShrink: 0 }} />
        <div style={{ fontFamily: "'Bebas Neue'", fontSize: 18, letterSpacing: 3, flexShrink: 0 }}>{group.name}</div>
        {/* Right column — 2 rows */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: "6px 4px", textAlign: "center", minWidth: 90 }} onClick={e => e.stopPropagation()}>
                        {(() => {
                          const logs = actionLogs.filter(l => l.holeIdx === i);
                          const isFutureMN = mnActive && slot >= currentSlot;
                          const isFutureTM = tmActive && slot >= currentSlot;
                          return (
                            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                              ...
                              {logs.map((l, li) => (
                                <div key={li} style={{ fontSize: 9, color: logColor(l.type), background: `${logBg(l.type)}44`, borderRadius: 4, padding: "2px 4px", textAlign: "left", lineHeight: 1.4 }}>
          ...
          <div style={{ background: "#141626", border: `1px solid ${logColor(actionModal.type)}88`, borderRadius: 14, padding: 28, minWidth: 300, boxShadow: "0 20px 60px #000" }}>
            ...
            {actionModal.type === "TM" && (
              <div style={{ marginBottom: 18 }}>
                ...
                <div style={{ fontSize: 10, color: "#555", marginTop: 6 }}>Multiple players can be selected, or press "All" if the whole group is being timed (groups have 2-4 players)</div>
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={confirmAction}
                style={{ flex: 1, background: logBg(actionModal.type), border: `1px solid ${logColor(actionModal.type)}`, color: logColor(actionModal.type), borderRadius: 8, padding: "10px", cursor: "pointer", fontFamily: "'Bebas Neue'", fontSize: 16, letterSpacing: 2 }}>
...
function Dashboard({ groups, groupData, pars, parTimes, schedules, onSelectGroup, onBack, currentUser,
  suspensions, isSuspended, pendingStopTime, totalOffsetMin, onSuspendStop, onSuspendResume, onLogout, onNavigateSummary }) {
  const [now, setNow] = useState(nowInMin());
    ...
    for (let s = 17; s >= 0; s--) {
      ...
      if (hd?.startTime && hd?.endTime) {
        ...
        if (hd.manualDiff !== undefined) {
          ...
          const [eh, em] = hd.endTime.split(":").map(Number);
          const actual = (eh * 60 + em) - (sh * 60 + sm);
          const parTime = parTimes?.[hi] ?? 14;
          diff = Math.round(actual - parTime) + 1;
        }
        return { holeIdx: hi, diff };
      }
    }
  ...
  const getGroupStatus = (g) => {
    const { diff } = getLastFinishedHole(g);
    if (diff === null) return "idle";
    return getStatus(diff);
  };
    ...
    <div style={{ minHeight: "100vh", background: "#0d0f1a", fontFamily: "'IBM Plex Mono', monospace", color: "#eee" }}>
      ...
      <div style={{ background: "#141626", borderBottom: "1px solid #2a2d4a", padding: "12px 24px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 5 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {onNavigateSummary && (
              <button
                ...
                style={{
                  background: "#0a1a2a", border: "1px solid #6effa088", color: "#6effa0",
                  borderRadius: 7, padding: "5px 12px", cursor: "pointer",
                  fontFamily: "'IBM Plex Mono'", fontSize: 11, fontWeight: 700, letterSpacing: 1,
                }}
              >📈 Summary</button>
            )}
            <button
              ...
              style={{
                background: "#0a1a2a", border: "1px solid #4e9af188", color: "#4e9af1",
                borderRadius: 7, padding: "5px 12px", cursor: "pointer",
                fontFamily: "'IBM Plex Mono'", fontSize: 11, fontWeight: 700, letterSpacing: 1,
              }}
            >⬇ Export Data</button>
          ...
          {/* Row 2: suspension button */}
          <div>
            {!isSuspended ? (
              <button onClick={openStop} style={{
                background: "#2a1500", border: "1px solid #ffd96688", color: "#ffd966",
                borderRadius: 7, padding: "5px 12px", cursor: "pointer",
                fontFamily: "'IBM Plex Mono'", fontSize: 11, fontWeight: 700, letterSpacing: 1,
              }}>⏸ Stopping play</button>
            ) : (
              <button onClick={openResume} style={{
                background: "#0a2a10", border: "1px solid #6effa088", color: "#6effa0",
                borderRadius: 7, padding: "5px 12px", cursor: "pointer",
                fontFamily: "'IBM Plex Mono'", fontSize: 11, fontWeight: 700, letterSpacing: 1,
      ...
      <div style={{ padding: "16px 20px" }}>
        ...
        {(() => {
          ...
          const renderGroupCard = (g) => {
            ...
            const diffColor = nowDiff === null ? "#555" : nowDiff >= 3 ? "#ff7070" : nowDiff >= 1 ? "#ffd966" : "#6effa0";
            const mnActive = gd?.mnActive === true;
                  ...
                  return (
                    <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 3, alignItems: "center" }}>
                      ...
                      {logs.map((l, li) => (
                        ...
                        }} title={`${l.type} H${l.holeIdx + 1}${l.target ? " · " + l.target : ""}${l.name ? " · " + l.name : ""}`}>
                          {l.type} H{l.holeIdx + 1}
                        </span>
                      ))}
                    </div>
                  );
                                  ...
                                  >
                                    ...
                                    {holeLogs.map((l, li) => (
                                      <div key={li} style={{ marginTop: 3, fontSize: 9, fontWeight: 700, color: logColor(l.type), background: `${logBg(l.type)}55`, borderRadius: 4, padding: "1px 4px" }}>
                                        {l.type} {l.name}{l.target ? ` · ${l.target}` : ""}
                                      </div>
                                    ))}
                                  </td>
                                );
                              }
                              const diff = hd.manualDiff !== undefined
                                ? hd.manualDiff
                                : (() => {
                                    const [sh, sm] = startTime.split(":").map(Number);
                                    const [eh, em] = endTime.split(":").map(Number);
                                    const actualMin = (eh * 60 + em) - (sh * 60 + sm);
                                    return Math.round(actualMin - (parTimes?.[hi] ?? 14)) + 1;
                                  })();
                              const displayEnd = hd.manualDiff !== undefined ? minToTime(deadline - 1 + hd.manualDiff) : endTime;
                              const color = diff >= 3 ? "#ff7070" : diff >= 1 ? "#ffd966" : "#6effa0";
                              return (
...
export default function App() {
  ...
  const handleStart = (grps, ps, pt) => {
    ...
    grps.forEach(g => {
      ...
      data[g.id] = { records: Array(18).fill(null), holeData: Array(18).fill(null).map(() => ({ startTime: null, endTime: null })), currentHole: 0 };
    });
    ...
    setPendingStopTime("");
    setScreen("dashboard");
    saveAppState({ groups: grps, pars: ps, parTimes: pt, baseSchedules: sch, schedules: sch, suspensions: [], isSuspended: false, pendingStopTime: "" });
  ...
  if (screen === "setup") return <SetupScreen onStart={handleStart} currentUser={currentUser} isAdmin={isAdmin} onManageUsers={() => setScreen("users")} onLogout={handleLogout} onClearSession={handleClearSession} />;
